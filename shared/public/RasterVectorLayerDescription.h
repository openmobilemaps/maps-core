...
enum class RasterTextureLookupMode {
    MONO,
    DUAL,
    QUAD,
};

struct RasterTextureLookup {
    SpriteIconId sprite;
    double zoomMin;
    double zoomMax;
    RasterTextureLookupMode mode;

    RasterTextureLookup(SpriteIconId sprite,
                        double zoomMin,
                        double zoomMax,
...
public:
    ...
    std::optional<RasterTextureLookup> textureLookup;
    ...
    , source(source)
    , style(style)
    , textureLookup(std::move(textureLookup)) {}

    std::unique_ptr<VectorLayerDescription> clone() override {
        auto cloned = std::make_unique<RasterVectorLayerDescription>(identifier,
                                                                     source,
                                                                     minZoom,
                                                                     maxZoom,
                                                                     filter,
