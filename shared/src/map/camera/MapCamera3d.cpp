...
float MapCamera3d::getRotation() { return cameraMode == CameraMode::POSE ? poseCameraState.yawDegrees : angle; }
...
void MapCamera3d::update() {
    inertiaStep();
    if (cameraMode == CameraMode::ORBIT && cameraZoomConfig.rotationSpeed) {
        std::lock_guard<std::recursive_mutex> lock(paramMutex);
...
bool MapCamera3d::onTwoFingerMove(const std::vector<::Vec2F> &posScreenOld, const std::vector<::Vec2F> &posScreenNew) {
    ...
    if (cameraMode == CameraMode::POSE) {
        if (posScreenOld.size() >= 2) {
            double scaleFactor =
                Vec2FHelper::distance(posScreenNew[0], posScreenNew[1]) / std::max(1.0, Vec2FHelper::distance(posScreenOld[0], posScreenOld[1]));
...
int32_t MapCamera3d::getCameraMode() { return static_cast<int32_t>(cameraMode); }

void MapCamera3d::setCameraMode(int32_t mode) {
    {
        std::lock_guard<std::recursive_mutex> lock(paramMutex);
        cameraMode = mode == static_cast<int32_t>(CameraMode::POSE) ? CameraMode::POSE : CameraMode::ORBIT;
        validVpMatrix = false;
    }
...
bool MapCamera3d::isPoseCameraActive() { return cameraMode == CameraMode::POSE; }
...
void MapCamera3d::clearPoseCamera() { setCameraMode(static_cast<int32_t>(CameraMode::ORBIT)); }
