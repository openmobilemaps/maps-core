// AUTOGENERATED FILE - DO NOT MODIFY!
// This file generated by Djinni from graphicsobjects.djinni

#import "GPHCRectangle2dInterface+Private.h"
#import "GPHCRectangle2dInterface.h"
#import "DJICppWrapperCache+Private.h"
#import "DJIError.h"
#import "DJIObjcWrapperCache+Private.h"
#import "GPHCGraphicsObjectInterface+Private.h"
#import "GPHCRectF+Private.h"
#import "GPHCRenderingContextInterface+Private.h"
#import "GPHCTextureHolder+Private.h"
#include <exception>
#include <stdexcept>
#include <utility>

static_assert(__has_feature(objc_arc), "Djinni requires ARC to be enabled for this file");

@interface GPHCRectangle2dInterfaceCppProxy : NSObject<GPHCRectangle2dInterface>

- (id)initWithCpp:(const std::shared_ptr<::Rectangle2dInterface>&)cppRef;

@end

@implementation GPHCRectangle2dInterfaceCppProxy {
    ::djinni::CppProxyCache::Handle<std::shared_ptr<::Rectangle2dInterface>> _cppRefHandle;
}

- (id)initWithCpp:(const std::shared_ptr<::Rectangle2dInterface>&)cppRef
{
    if (self = [super init]) {
        _cppRefHandle.assign(cppRef);
    }
    return self;
}

- (void)setFrame:(nonnull GPHCRectF *)frame
textureCoordinates:(nonnull GPHCRectF *)textureCoordinates {
    try {
        _cppRefHandle.get()->setFrame(::djinni_generated::RectF::toCpp(frame),
                                      ::djinni_generated::RectF::toCpp(textureCoordinates));
    } DJINNI_TRANSLATE_EXCEPTIONS()
}

- (void)loadTexture:(nullable id<GPHCRenderingContextInterface>)context
      textureHolder:(nullable id<GPHCTextureHolder>)textureHolder {
    try {
        _cppRefHandle.get()->loadTexture(::djinni_generated::RenderingContextInterface::toCpp(context),
                                         ::djinni_generated::TextureHolder::toCpp(textureHolder));
    } DJINNI_TRANSLATE_EXCEPTIONS()
}

- (void)removeTextures:(nullable id<GPHCRenderingContextInterface>)context {
    try {
        _cppRefHandle.get()->removeTextures(::djinni_generated::RenderingContextInterface::toCpp(context));
    } DJINNI_TRANSLATE_EXCEPTIONS()
}

- (nullable id<GPHCGraphicsObjectInterface>)getAsGraphicsObject {
    try {
        auto objcpp_result_ = _cppRefHandle.get()->getAsGraphicsObject();
        return ::djinni_generated::GraphicsObjectInterface::fromCpp(objcpp_result_);
    } DJINNI_TRANSLATE_EXCEPTIONS()
}

namespace djinni_generated {

class Rectangle2dInterface::ObjcProxy final
: public ::Rectangle2dInterface
, private ::djinni::ObjcProxyBase<ObjcType>
{
    friend class ::djinni_generated::Rectangle2dInterface;
public:
    using ObjcProxyBase::ObjcProxyBase;
    void setFrame(const ::RectF & c_frame, const ::RectF & c_textureCoordinates) override
    {
        @autoreleasepool {
            [djinni_private_get_proxied_objc_object() setFrame:(::djinni_generated::RectF::fromCpp(c_frame))
                                            textureCoordinates:(::djinni_generated::RectF::fromCpp(c_textureCoordinates))];
        }
    }
    void loadTexture(const std::shared_ptr<::RenderingContextInterface> & c_context, const std::shared_ptr<::TextureHolder> & c_textureHolder) override
    {
        @autoreleasepool {
            [djinni_private_get_proxied_objc_object() loadTexture:(::djinni_generated::RenderingContextInterface::fromCpp(c_context))
                                                    textureHolder:(::djinni_generated::TextureHolder::fromCpp(c_textureHolder))];
        }
    }
    void removeTextures(const std::shared_ptr<::RenderingContextInterface> & c_context) override
    {
        @autoreleasepool {
            [djinni_private_get_proxied_objc_object() removeTextures:(::djinni_generated::RenderingContextInterface::fromCpp(c_context))];
        }
    }
    std::shared_ptr<::GraphicsObjectInterface> getAsGraphicsObject() override
    {
        @autoreleasepool {
            auto objcpp_result_ = [djinni_private_get_proxied_objc_object() getAsGraphicsObject];
            return ::djinni_generated::GraphicsObjectInterface::toCpp(objcpp_result_);
        }
    }
};

}  // namespace djinni_generated

namespace djinni_generated {

auto Rectangle2dInterface::toCpp(ObjcType objc) -> CppType
{
    if (!objc) {
        return nullptr;
    }
    if ([(id)objc isKindOfClass:[GPHCRectangle2dInterfaceCppProxy class]]) {
        return ((GPHCRectangle2dInterfaceCppProxy*)objc)->_cppRefHandle.get();
    }
    return ::djinni::get_objc_proxy<ObjcProxy>(objc);
}

auto Rectangle2dInterface::fromCppOpt(const CppOptType& cpp) -> ObjcType
{
    if (!cpp) {
        return nil;
    }
    if (auto cppPtr = dynamic_cast<ObjcProxy*>(cpp.get())) {
        return cppPtr->djinni_private_get_proxied_objc_object();
    }
    return ::djinni::get_cpp_proxy<GPHCRectangle2dInterfaceCppProxy>(cpp);
}

}  // namespace djinni_generated

@end
