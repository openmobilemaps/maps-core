...
actual open class MapCameraInterface actual constructor(nativeHandle: Any?) {
	...
	actual fun moveToBoundingBox(boundingBox: RectCoord, paddingPc: Float, animated: Boolean, minZoom: Double?, maxZoom: Double?) {
		val minZoomNumber = minZoom?.let { NSNumber(double = it) }
		val maxZoomNumber = maxZoom?.let { NSNumber(double = it) }
		camera?.moveToBoundingBox(boundingBox, paddingPc, animated, minZoomNumber, maxZoomNumber)
	}
	...
	actual fun getInvariantModelMatrix(coordinate: Coord, scaleInvariant: Boolean, rotationInvariant: Boolean): List<Float> {
		val list = camera?.getInvariantModelMatrix(coordinate, scaleInvariant, rotationInvariant) ?: return emptyList()
		return list.mapNotNull { (it as? NSNumber)?.floatValue }
	}
	...
	actual fun coordFromScreenPosition(posScreen: Vec2F): Coord = requireNotNull(camera?.coordFromScreenPosition(posScreen))
	...
	actual fun screenPosFromCoord(coord: Coord): Vec2F = requireNotNull(camera?.screenPosFromCoord(coord))
	...
	actual fun getLastVpMatrixD(): List<Double>? =
		camera?.getLastVpMatrixD()?.mapNotNull { (it as? NSNumber)?.doubleValue }

	actual fun getLastVpMatrix(): List<Float>? =
		camera?.getLastVpMatrix()?.mapNotNull { (it as? NSNumber)?.floatValue }

	actual fun getLastInverseVpMatrix(): List<Float>? =
		camera?.getLastInverseVpMatrix()?.mapNotNull { (it as? NSNumber)?.floatValue }

	actual fun getLastVpMatrixViewBounds(): RectCoord? = camera?.getLastVpMatrixViewBounds()

	actual fun getLastVpMatrixRotation(): Float? = camera?.getLastVpMatrixRotation()?.floatValue

	actual fun getLastVpMatrixZoom(): Float? = camera?.getLastVpMatrixZoom()?.floatValue
...
private class MapCameraListenerProxy(
	...
	override fun onCameraChange(
		...
		val projection = projectionMatrix.mapNotNull { (it as? NSNumber)?.floatValue }
