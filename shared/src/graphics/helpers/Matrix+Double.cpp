 ...
 */
void Matrix::transposeM(std::vector<double> &mTrans, int mTransOffset, std::vector<double> &m, int mOffset) {
    for (int i = 0; i < 4; i++) {
 ...
 */
bool Matrix::invertM(std::vector<double> &mInv, int mInvOffset, std::vector<double> &m, int mOffset) {
 ...
 */
void Matrix::orthoM(std::vector<double> &m, int mOffset, double left, double right, double bottom, double top, double near, double far) {
 ...
 */
void Matrix::frustumM(std::vector<double> &m, int offset, double left, double right, double bottom, double top, double near, double far) {
    double r_width = 1.0f / (right - left);
    double r_height = 1.0f / (top - bottom);
 ...
 */
void Matrix::perspectiveM(std::vector<double> &m, int offset, double fovy, double aspect, double zNear, double zFar) {
 ...
 */
double Matrix::length(double x, double y, double z) { return (double)std::sqrt(x * x + y * y + z * z); }
 ...
 */
void Matrix::setIdentityM(std::vector<double> &sm, int smOffset) {
    for (int i = 0; i < 16; i++) {
        sm[smOffset + i] = 0;
    }
 ...
 */
void Matrix::scaleM(std::vector<double> &sm, int smOffset, std::vector<double> &m, int mOffset, double x, double y, double z) {
    for (int i = 0; i < 4; i++) {
        int smi = smOffset + i;
        int mi = mOffset + i;
        sm[smi] = m[mi] * x;
 ...
 * @param z zoom factor z
 */
void Matrix::scaleM(std::vector<double> &m, int mOffset, double x, double y, double z) {
    for (int i = 0; i < 4; i++) {
        int mi = mOffset + i;
        m[mi] *= x;
        m[4 + mi] *= y;
 ...
 */
void Matrix::translateM(std::vector<double> &tm, int tmOffset, std::vector<double> &m, int mOffset, double x, double y, double z) {
    for (int i = 0; i < 12; i++) {
 ...
 * @param z translation factor z
 */
void Matrix::translateM(std::vector<double> &m, int mOffset, double x, double y, double z) {
    for (int i = 0; i < 4; i++) {
        int mi = mOffset + i;
 ...
 */
void Matrix::setLookAtM(std::vector<double> &rm, int rmOffset, double eyeX, double eyeY, double eyeZ, double centerX, double centerY,
 ...
 */
void Matrix::rotateM(std::vector<double> &m, int mOffset, double a, double x, double y, double z) {
    std::vector<double> sTemp = std::vector<double>(32, 0);
 ...
 * @param z Z axis component
 */
void Matrix::setRotateM(std::vector<double> &rm, int rmOffset, double a, double x, double y, double z) {
    rm[rmOffset + 3] = 0;
    rm[rmOffset + 7] = 0;
    rm[rmOffset + 11] = 0;
    rm[rmOffset + 12] = 0;
...
void Matrix::multiplyMM(std::vector<double> &r, int resultOffset, std::vector<double> &lhs, int lhsOffset, std::vector<double> &rhs,
                        int rhsOffset) {
    multiplyMMC(r, resultOffset, lhs, lhsOffset, rhs, rhsOffset);
}

void Matrix::multiplyMMC(std::vector<double> &r, int resultOffset, const std::vector<double> &lhs, int lhsOffset,
...
std::vector<double> Matrix::multiply(const std::vector<double> &M, const std::vector<double> &x) {
...
void Matrix::multiply(const std::vector<double> &M, const std::vector<double> &x, std::vector<double> &result) {
    for (int i = 0; i < 4; i++) {
        result[i] = 0.0;
        for (int j = 0; j < 4; j++) {
            result[i] += M[I(j, i)] * x[j];
