...
class FontLoader(context: Context, private val dpFactor: Float) : FontLoaderInterface() {
    ...
    // TODO: Fonts for current vector map. Remove on change/release
    init {
        listOf("fonts/FrutigerLTStd-Roman", "fonts/Frutiger Neue LT Condensed Bold").forEach { fontPath ->
            val jsonString =
                context.resources.assets.open("$fontPath.json").bufferedReader().use { it.readText() }
            val fontAtlas = BitmapFactory.decodeStream(context.resources.assets.open("$fontPath.png"))
    ...
    fun addFont(fontDataJson: String, fontAtlas: Bitmap) {
        ...
        fontMap[fontJson.name] = FontDataHolder(BitmapTextureHolder(fontAtlas), FontData(fontInfo, glyphs))
    }
