...
final class Quad2dTessellatedDisplaced: BaseGraphicsObject, @unchecked Sendable {
    ...
    private func setupDepthStencilStates() {
        ...
        // Displaced terrain still clips against geometry-mask bits while keeping depth testing enabled.
        stencil.readMask = 0b1100_0000
        stencil.writeMask = 0b0000_0000

        let depthStencil = MTLDepthStencilDescriptor()
        depthStencil.depthCompareFunction = .lessEqual
        depthStencil.isDepthWriteEnabled = true
        depthStencil.frontFaceStencil = stencil
        depthStencil.backFaceStencil = stencil

        depthStencilState = device.makeDepthStencilState(descriptor: depthStencil)
    }
    ...
    override func render(
        ...
        if isMasked {
            // The masked displaced draw combines the shared stencil read with the model's depth state.
            // Inverse reads target the cleared value for outside-of-mask rendering.
            encoder.setDepthStencilState(maskDepthStencilState(for: renderPass))
            encoder.setStencilReferenceValue(maskStencilReference(for: renderPass))
        } else if renderPass.isPassMasked {
            // Self-masked displaced passes retain depth writes and use only the low stencil bit.
            ...
            encoder.setDepthStencilState(context.defaultMaskDepth)
        }
        ...
        // The displaced quad renders with the raster fragment shader, which samples the lookup sprite
        // at fragment texture index 1 when the raster style is in texture-lookup mode. Bind the lookup
        // texture when present, otherwise fall back to the color texture to keep the binding satisfied.
        ...
        // Elevation is sampled in the vertex/tessellation stage; it lives in a separate texture namespace
        // from the fragment lookup, so the two never collide on Metal.
...
extension Quad2dTessellatedDisplaced: MCQuad2dInterface {
    ...
    func loadTextures(
        ...
        let lookupHolder = lookupHolder as? TextureHolder
        let elevationHolder = elevationHolder as? TextureHolder
        lock.withCritical {
            texture = textureHolder.texture
    ...
    func asMaskingObject() -> MCMaskingObjectInterface? {
        fatalError("Don't use this as a mask.")
    }
}
