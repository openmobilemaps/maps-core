...
class LineGeometryBuilder {
  public:
    static void buildLines(const std::shared_ptr<LineGroup2dInterface> &line,
        ...
        const size_t reservedLineAttributes = lineAttributes.capacity();
        const size_t reservedLineIndices = lineIndices.capacity();
  ...
  private:
                                            ...
                                            std::vector<float> &lineAttributes, std::vector<uint32_t> &lineIndices)
    {
        ...
        for(auto &[lineCoords, lineStyleIndex]  : lines) {
            ...
            numTriangles += numPoints*numTrianglePointFactor + numTriangleOffset + capVertices;
        }
