...
class LineGroup2dLayerObject : public LayerObjectInterface {
  public:
    ...
    void setLines(const std::vector<std::tuple<std::vector<Vec2D>, int>> &lines, const int32_t systemIdentifier, const Vec3D & origin, LineCapType capType = LineCapType::BUTT);

    void setLines(const std::vector<std::tuple<std::vector<Coord>, int>> &lines, const Vec3D & origin, LineCapType capType = LineCapType::BUTT);
