    ...
    public class ImmersiveMapRenderer {
        ...
        public func run() {
            ...
            Task(executorPreference: RendererTaskExecutor.shared) {
                ...
                let layerRenderer = await self.layerRenderer
                ...
                while true {
                    ...
                    if await self.followHand, let handAnchor = handTracking.latestAnchors.rightHand {
                        handOffset = handOffset * 0.99 + handAnchor.originFromAnchorTransform * 0.01
                    }
                    ...
                    for (eye, _) in drawable.colorTextures.enumerated() {
                        ...
                        if let originFromDevice {
                            ...
                            for column in 0..<4 {
                                for row in 0..<4 {
                                    flattenedViewMatrix.append(
                                        NSNumber(value: viewMatrix[column, row])
                                    )
                                }
                            }
                            ...
                            for column in 0..<4 {
                                for row in 0..<4 {
                                    flattenedProjectionMatrix.append(
                                        NSNumber(value: projection[column, row])
                                    )
                                }
                            }
