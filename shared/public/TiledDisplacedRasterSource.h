...
class TiledDisplacedRasterSource : public Tiled2dMapRasterSource {
  public:
    TiledDisplacedRasterSource(const MapConfig &mapConfig, const std::shared_ptr<Tiled2dMapLayerConfig> &layerConfig,
                               const std::shared_ptr<Tiled2dMapLayerConfig> &elevationConfig,
                               const std::shared_ptr<CoordinateConversionHelperInterface> &conversionHelper,
                               const std::shared_ptr<SchedulerInterface> &scheduler,
                               const std::vector<std::shared_ptr<::LoaderInterface>> &loaders,
                               const WeakActor<Tiled2dMapRasterSourceListener> &listener, float screenDensityPpi,
                               std::string layerName);

  protected:
    using RasterSourceBase = Tiled2dMapSource<std::shared_ptr<TextureLoaderResult>, std::shared_ptr<::TextureHolderInterface>>;
    ...
    const Tiled2dMap3dTileSelection<RasterSourceBase> &get3dTileSelection() const override;
  ...
  protected:
    ...
    const std::shared_ptr<Tiled2dMapLayerConfig> elevationConfig;
    std::unique_ptr<Tiled2dMap3dTileSelection<RasterSourceBase>> displacedTileSelection;
