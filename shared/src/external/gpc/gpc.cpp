...
void gpc_set_polygon (const std::vector<std::vector<::Coord>>& orig, gpc_polygon *p)
{
    p->num_contours = (int)orig.size();
	...
	for (unsigned int i = 0; i < orig.size(); i++) {
		p->contour[i].num_vertices = (int)orig.at(i).size();
		...
		for (int v = 0; v < p->contour[i].num_vertices; v++) {
			p->contour[i].vertex[v].x = orig.at(i).at(v).x;
			p->contour[i].vertex[v].y = orig.at(i).at(v).y;
		}
	}
}

void gpc_set_polygon (const PolygonCoord& p, gpc_polygon *polygon) {
    polygon->num_contours = (p.positions.empty() ? 0 : 1) + (int)p.holes.size();
    MALLOC(polygon->hole, polygon->num_contours * sizeof(int), "hole flag array creation", int);
    MALLOC(polygon->contour, polygon->num_contours * sizeof(gpc_vertex_list), "contour creation", gpc_vertex_list);

    if (!p.positions.empty()) {
        polygon->contour[0].num_vertices = (int)p.positions.size();
        polygon->hole[0]= FALSE; /* Assume the contours to be external */
        MALLOC(polygon->contour[0].vertex, polygon->contour[0].num_vertices * sizeof(gpc_vertex), "vertex creation", gpc_vertex);
        for (int v = 0; v < polygon->contour[0].num_vertices; v++) {
            polygon->contour[0].vertex[v].x = p.positions.at(v).x;
            polygon->contour[0].vertex[v].y = p.positions.at(v).y;
        }
    }


    for (unsigned int i = 0; i < p.holes.size(); i++) {
        polygon->contour[i + 1].num_vertices = (int)p.holes.at(i).size();
        polygon->hole[i + 1]= FALSE; /* Assume the contours to be external */
        MALLOC(polygon->contour[i + 1].vertex, polygon->contour[i + 1].num_vertices * sizeof(gpc_vertex), "vertex creation", gpc_vertex);
        for (int v = 0; v < polygon->contour[i + 1].num_vertices; v++) {
            polygon->contour[i + 1].vertex[v].x = p.holes.at(i).at(v).x;
            polygon->contour[i + 1].vertex[v].y = p.holes.at(i).at(v).y;
        }
    }
}

PolygonCoord gpc_get_polygon_coord (gpc_polygon *polygon, const std::string &systemIdentifier) {
    ...
    for (int c = 0; c < polygon->num_contours; c++) {
        ...
        for (int v = 0; v < polygon->contour[c].num_vertices; v++) {
            p.push_back(Coord(systemIdentifier, polygon->contour[c].vertex[v].x, polygon->contour[c].vertex[v].y, 0));
        }
        p.push_back(Coord(systemIdentifier, polygon->contour[c].vertex[0].x, polygon->contour[c].vertex[0].y, 0));
    ...
    return PolygonCoord(positions, holes);
}
