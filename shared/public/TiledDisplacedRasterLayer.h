...
protected:
    ...
    void loadTileTextures(const std::shared_ptr<Textured2dLayerObject> &tileObject,
                          const std::shared_ptr<RenderingContextInterface> &renderingContext,
                          const Tiled2dMapRasterTileInfo &tileInfo) override;

    bool useRenderPassMaskingFor3d() const override;

    void setSourceMaskTileGeometryTileSelectionOptimization(bool enabled) override;
...
private:
    TiledDisplacedRasterLayer(std::pair<std::shared_ptr<::Tiled2dMapLayerConfig>, std::shared_ptr<::Tiled2dMapLayerConfig>> limitedConfigs,
                              const std::vector<std::shared_ptr<::LoaderInterface>> & tileLoaders,
