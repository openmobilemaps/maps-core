...
private:
    ...
    const Tiled2dMapZoomInfo defaultZoomInfo = Tiled2dMapZoomInfo(1.0, 0, false, true);
    ...
    virtual std::vector<Tiled2dMapZoomLevelInfo> getDefaultEpsg3857ZoomLevels() {
        ...
        for (int i = description->minZoom; i <= description->maxZoom; i++) {
            ...
            infos.push_back(Tiled2dMapZoomLevelInfo(zoom, width, factor, factor, i, epsg3857Bounds));
        }
        return infos;
    }
};
