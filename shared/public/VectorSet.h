...
class StringVectorSet {
public:
    StringVectorSet() {};
    
    // Constructor with initializer list
    StringVectorSet(std::initializer_list<std::string> initList) {
    ...
    bool covers(const StringVectorSet& other) const {
    ...
    void insertCollection(const StringVectorSet& otherSet) {
        for (auto it = otherSet.data.begin(); it != otherSet.data.end(); it++) {
            insert(*it);
        }
    }
